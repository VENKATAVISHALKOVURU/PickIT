  ...
  useEffect(() => {
    if (scanState === 'SCANNING') {
      ...
      html5QrCode.start(
        ...
        config,
        (decodedText) => {
          // Supports both SHOP-1234 (Generated) and SHOP-A1B2C3 (Legacy/External)
          const match = decodedText.match(/SHOP-[A-Z0-9]{4,8}/);
          if (match) {
            handleScanSuccess(match[0]);
          } else if (decodedText.includes('SHOP-')) {
            // Fallback loose match
            const idMatch = decodedText.match(/SHOP-[A-Z0-9]+/);
